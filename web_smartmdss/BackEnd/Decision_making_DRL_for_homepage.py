...
def execute_DRL_and_noIntervention():
    #
    ...
    collection = db['Pulled_data_past48h']
    ##################################   With Intervention  #########################################

    for document in collection.find():
        forecast_data = document.get('forecast', [])[:33]  # Get only the first 30 hours of forecast
        snow_precipitation_detected = any(item.get('snow', 0) > 0 for item in forecast_data)
        if snow_precipitation_detected:
            print("Snow precipitation detected in the first 30 hours. Triggering DRL model...")

    # Fetch data from MongoDB
            #data = collection.find_one({'forecast': {'$exists': True}})  # Assuming one document format
            df = pd.DataFrame(forecast_data)

            df['MeasureTime'] = pd.to_datetime(df['time'])
            df['Air TemperatureC'] = (df['temp'] - 32) * 5.0 / 9.0  # F to C
            df['Surface TemperatureC'] = (df['surface_temp'] - 32) * 5.0 / 9.0  # F to C
            df['Wind Speed (act)m/s'] = df['wind_speed'] * 0.44704  # mph to m/s
            ...
            df.drop(['temp', 'surface_temp', 'wind_speed', 'snow', 'rain'], axis=1, inplace=True)
            ...
            df_resampled = df.resample('3H', origin=first_time).agg({
                'Air TemperatureC': 'mean',
                'Surface TemperatureC': 'mean',
                'Wind Speed (act)m/s': 'mean',
                'Precipitation Intensitym/3h': 'sum'
            }).reset_index()

            # Add the window ID (assuming it is constant as 'Appleton_window_1')
            df_resampled['Window_ID'] = 'This'
    ...
    # Reorder columns to match the desired output
            ...
            for model_file in models:
                ...
                current_episode_data =  dataset[dataset['Window_ID'] == 'This']
            ...
            if maintenance_suggestions:  # Check if there are any suggestions to add
                ...
                result = collection.update_one(
                    {'_id': document['_id']},  # Assuming this is how you identify the document
